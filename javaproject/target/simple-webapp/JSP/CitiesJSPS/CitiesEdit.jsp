...
<body>
	<%
		if(request.getParameter("cityno")!=null)
		{
			Cities	city=new CitiesService().find(Integer.parseInt(request.getParameter("cityno")));
  ...
  <form action="CitiesEdit.jsp?cityno=${param.cityno}" method="post" class="flex-grow-1">
        ...
        <div class="col-md-6 offset-md-3">
          <div class="custom-input-group">
              <select class="form-select custom-input" name="select" id="select">
                ...
                %>
                <option value="${city.state.stateNo}">${city.state.stateName}</option>
                <c:forEach items="${States}" var="temp">
                  <option value="${temp.stateNo}">${temp.stateName}</option>
                </c:forEach>
              </select>
              <input type="text" class="form-control custom-input" placeholder="Enter State" aria-label="Enter State" name="t1" id="t1" value="${city.cityName }">
    ...
    <c:when test="${param.btn == 'Edit'}">
      <c:if test="${param.t1 != 'null'}">
        <jsp:useBean id="cities" class="com.iss.models.Cities" scope="request">
                ...
                <jsp:setProperty name="cities" property="cityNo" value="${param.cityno}" />
            </jsp:useBean>
                <% 
                    try {
                    	States state=new StatesService().find(Integer.parseInt(request.getParameter("select")));
                    	cities.setState(state);
                        new CitiesService().edit(cities);
                        response.sendRedirect("CitiesEdit.jsp");
                    } catch(Exception ex) {
                        ex.printStackTrace();        
                    }
                %>
