...
<body>
	<%
			...
			{
				States	state=new StatesService().find(Integer.parseInt(request.getParameter("stateid")));
				request.setAttribute("state",state);
			}
	%>
  <nav class="navbar navbar-expand-sm bg-dark navbar-dark">
    <div class="container-fluid">
      <a class="navbar-brand" href="#">StatesDetails</a>
      <ul class="navbar-nav ms-auto">
        ...
        <li class="nav-item">
          <a class="nav-link active" href="States.jsp"><img src="https://th.bing.com/th/id/OIP.D5Npngc53EqWc9tZ6VFytAHaHa?rs=1&pid=ImgDetMain" alt="Logo" style="width:40px;" class="rounded-pill"></a>
        </li>
        <li class="nav-item">
  ...
  <form action="StatesEdit.jsp?stateid=${param.stateid}" method="post" class="flex-grow-1">
        ...
        <div class="col-md-6 offset-md-3">
          <div class="custom-input-group">
              <select class="form-select custom-input" name="select" id="select">
                <%
                  List<Countries> list = new CountriesService().getAll();
                  request.setAttribute("countries", list);
                %>
                <option value="${state.country.countryNo}">${state.country.country}</option>
                <c:forEach items="${countries}" var="temp">
                  <option value="${temp.countryNo}">${temp.country}</option>
                </c:forEach>
              </select>
              <input type="text" class="form-control custom-input" placeholder="Enter State" aria-label="Enter State" name="t1" id="t1" value="${state.stateName }">
    ...
    <c:when test="${param.btn == 'Edit'}">
      <c:if test="${param.t1 != 'null'}">
        <jsp:useBean id="states" class="com.iss.models.States" scope="request">
                <jsp:setProperty name="states" property="stateName" value="${param.t1}" />
                <jsp:setProperty name="states" property="stateNo" value="${param.stateid}" />
            </jsp:useBean>
                <% 
	                try {
	                	Countries country=new CountriesService().find(Integer.parseInt(request.getParameter("select")));
	                	states.setCountry(country);
	                    new StatesService().edit(states);
	                    response.sendRedirect("StatesEdit.jsp");
	                } catch(Exception ex) {
	                    ex.printStackTrace();        
	                }
                %>
