...
<nav class="navbar navbar-expand-sm bg-dark navbar-dark">
    <div class="container-fluid">
        <a class="navbar-brand" href="#">VehiclesDetails</a>
        <ul class="navbar-nav ms-auto">
            ...
            <li class="nav-item">
                <a class="nav-link active" href="VehiclesAdd.jsp"><img src="https://icons4web.com/wp-content/uploads/2017/11/Add-internet-icon-190282.jpg" alt="Logo" style="width:40px;" class="rounded-pill"></a>
            </li>
            <li class="nav-item">
...
<div class="content">
    <div class="row">
        <div class="col-sm-12 bg-primary text-white">
            <div class="search-bar">
                <form class="d-flex" action="Vehicles.jsp?action=search" method="post">
    ...
    <c:choose>
        <c:when test="${param.btn == 'delete'}">
            <c:if test="${param.t1 != 'null'}">
                <% 
                try {
                    new VehiclesService().delete(Integer.parseInt(request.getParameter("vehicleno")));
        ...
        <c:when test="${param.btn=='search'}">
            <c:if test="${param.search!='null'}">
                <%
                try {
                    List<Vehicles> l = new VehiclesService().search(request.getParameter("search"));
                    request.setAttribute("listVehicles", l);
                } catch(Exception ex) {
                    ex.printStackTrace();		
                }
                %>
        ...
        <c:when test="${param.btn=='edit'}">
            <%
            try {
                request.getRequestDispatcher("VehiclesEdit.jsp").forward(request, response);
            } catch(Exception ex) {
                ex.printStackTrace();		
            }
            %>
        ...
        <c:otherwise>
            <%
            List<Vehicles> list = new VehiclesService().getAll();
            request.setAttribute("listVehicles", list);
            %>
        </c:otherwise>
    </c:choose>
    <div class="container mt-3 table-responsive" style="height:320px"> 
        <table class="table table-striped table-bordered table-dark">
            <thead>
                <tr>
                    <th class="col s2">Reg.No</th>
                    <th class="col s2">Reg.State</th>
                    <th class="col s2">Type</th>
                    <th class="col s2">Model</th>
                    <th class="col s2">Fuel</th>
                    <th class="col s2">Capacity</th>
                    <th class="col s2">Rate</th>
                    <th></th>
                </tr>
            </thead>
            <tbody>
                <c:forEach items="${listVehicles}" var="temp">
                    <tr>
                        <div >
                            <td class="col s2">${temp.registrationNumber}</td>
                            <td class="col s2">${temp.state.stateName}</td> 
                              <td class="col s2">${temp.type}</td>
                               <td class="col s2">${temp.vehiclemodels.name}</td> 
                                <td class="col s2">${temp.fuel}</td> 
                                <td class="col s2">${temp.capacity}</td> 
                                <td class="col s2">${temp.dailyRate}</td>      
                        </div>
                        <div>
                            <td class="col-1">
                                <form action="Vehicles.jsp?vehicleno=${temp.vehicleNo }" method="post" style="display:inline;">
