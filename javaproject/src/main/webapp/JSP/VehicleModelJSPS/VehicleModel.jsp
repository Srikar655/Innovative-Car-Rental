...
<nav class="navbar navbar-expand-sm bg-dark navbar-dark">
    <div class="container-fluid">
        <a class="navbar-brand" href="#">VehicleModelDetails</a>
        <ul class="navbar-nav ms-auto">
            ...
            <li class="nav-item">
                <a class="nav-link active" href="VehicleModelAdd.jsp"><img src="https://icons4web.com/wp-content/uploads/2017/11/Add-internet-icon-190282.jpg" alt="Logo" style="width:40px;" class="rounded-pill"></a>
            </li>
            <li class="nav-item">
...
<div class="content">
    <div class="row">
        <div class="col-sm-12 bg-primary text-white">
            <div class="search-bar">
                <form class="d-flex" action="VehicleModel.jsp?action=search" method="post">
    ...
    <c:choose>
        <c:when test="${param.btn == 'delete'}">
            <c:if test="${param.t1 != 'null'}">
                <% 
                try {
                    new VehicleModelService().delete(Integer.parseInt(request.getParameter("vehiclemodelno")));
        ...
        <c:when test="${param.btn=='search'}">
            <c:if test="${param.search!='null'}">
                <%
                try {
                    List<VehicleModels> l = new VehicleModelService().search(request.getParameter("search"));
                    request.setAttribute("listModels", l);
                } catch(Exception ex) {
                    ex.printStackTrace();		
                }
                %>
        ...
        <c:when test="${param.btn=='edit'}">
            <%
            try {
                request.getRequestDispatcher("VehicleModelEdit.jsp").forward(request, response);
            } catch(Exception ex) {
                ex.printStackTrace();		
            }
            %>
        ...
        <c:otherwise>
            <%
            List<VehicleModels> list = new VehicleModelService().getAll();
            request.setAttribute("listModels", list);
            %>
        </c:otherwise>
    </c:choose>
    <div class="container mt-3 table-responsive" style="height:320px">
        <table class="table table-striped table-bordered table-dark">
            <thead>
                <tr>
                    <th class="col s6">VehicleModel</th>
                    <th class="col s6">VehicleMake</th>
                    <th></th>
                </tr>
            </thead>
            <tbody>
                <c:forEach items="${listModels}" var="temp">
                    <tr>
                        <div>
                            <td class="col s5">${temp.name}</td>
                            <td class="col s5">${temp.vehiclemakes.name}</td>    
                        </div>
                        <div>
                            <td class="col-2">
                                <form action="VehicleModel.jsp?vehiclemodelno=${temp.modelNo}" method="post" style="display:inline;">
