...
<body>
  ...
  <div class="content">
    <form action="VehicleModelAdd.jsp" method="post">
            ...
            <div class="custom-input-group">
              <select class="form-select custom-input" name="select" id="select">
                <%
                  ...
                  request.setAttribute("vechiclemakes", list);
                %>
                <option>--Select Vehicle Make--</option>
                <c:forEach items="${vechiclemakes}" var="temp">
                  <option value="${temp.makeNo}">${temp.name}</option>
                </c:forEach>
              </select>
              <input type="text" class="form-control custom-input" placeholder="Enter Vehicle Model" aria-label="Enter Vehicle Model" name="t1" id="t1">
      ...
      <c:when test="${param.btn == 'add'}">
        <c:if test="${param.t1 != 'null'}">
         <jsp:useBean id="vehiclemodels" class="com.iss.models.VehicleModels" scope="request">
                ...
                <% 
                    try {
                    	VehicleMakes vm=new VehicleMakesService().find(Integer.parseInt(request.getParameter("select")));
                    	vehiclemodels.setVehiclemakes(vm);
                        new VehicleModelService().add(vehiclemodels);
                        response.sendRedirect("VehicleModelAdd.jsp");
                    } catch(Exception ex) {
                        ex.printStackTrace();        
                    }
                %>
