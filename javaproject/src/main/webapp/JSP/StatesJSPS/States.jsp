...
<nav class="navbar navbar-expand-sm bg-dark navbar-dark">
    <div class="container-fluid">
        ...
        <ul class="navbar-nav ms-auto">
            ...
            <li class="nav-item">
                <a class="nav-link active" href="StatesAdd.jsp"><img src="https://icons4web.com/wp-content/uploads/2017/11/Add-internet-icon-190282.jpg" alt="Logo" style="width:40px;" class="rounded-pill"></a>
            </li>
            <li class="nav-item">
...
<div class="content">
    <div class="row">
        <div class="col-sm-12 bg-primary text-white">
            <div class="search-bar">
                <form class="d-flex" action="States.jsp?action=search" method="post">
    ...
    <c:choose>
        <c:when test="${param.btn == 'delete'}">
            <c:if test="${param.conid != 'null'}">
                <% 
                try {
                   	 new StatesService().delete(Integer.parseInt(request.getParameter("stateid")));
        ...
        <c:when test="${param.btn=='search'}">
            <c:if test="${param.search!='null'}">
                <%
                try {
                    List<States> l = new StatesService().search(request.getParameter("search"));
                    request.setAttribute("listStates", l);
                } catch(Exception ex) {
                    ex.printStackTrace();		
                }
                %>
        ...
        <c:when test="${param.btn=='edit'}">
            <%
            try {
                request.getRequestDispatcher("listStates.jsp").forward(request, response);
            } catch(Exception ex) {
                ex.printStackTrace();		
            }
            %>
        ...
        <c:otherwise>
            ...
            request.setAttribute("listStates", list);
            %>
        </c:otherwise>
    </c:choose>
    <div class="container mt-3 table-responsive" style="height:320px">
        <table class="table table-striped table-bordered table-dark">
            <thead>
                <tr>
                    <th scope="col-12">State Name</th>
                    <th></th>
                </tr>
            </thead>
            <tbody>
                <c:forEach items="${listStates}" var="temp">
                    <tr>
                        <div>
                            <td class="col-11">${temp.stateName}</td>
                        </div>
                        <div>
                            <td class="col-1">
                                <form action="States.jsp?stateid=${temp.stateNo}" method="post" style="display:inline;">
