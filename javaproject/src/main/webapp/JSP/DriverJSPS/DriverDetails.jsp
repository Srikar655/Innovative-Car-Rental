...
<nav class="navbar navbar-expand-sm bg-dark navbar-dark">
    <div class="container-fluid">
        ...
        <ul class="navbar-nav ms-auto">
            ...
            <li class="nav-item">
                <a class="nav-link active" href="DriverDetailsAdd.jsp"><img src="https://icons4web.com/wp-content/uploads/2017/11/Add-internet-icon-190282.jpg" alt="Logo" style="width:40px;" class="rounded-pill"></a>
            </li>
            <li class="nav-item">
...
<div class="content">
    <div class="row">
        <div class="col-sm-12 bg-primary text-white">
            <div class="search-bar">
                <form class="d-flex" action="DriverDetails.jsp?action=search" method="post">
    ...
    <c:choose>
        <c:when test="${param.btn == 'delete'}">
            <c:if test="${param.t1 != 'null'}">
                <% 
                try {
                    new DriverDetailsService().delete(Integer.parseInt(request.getParameter("driverno")));
        ...
        <c:when test="${param.btn=='search'}">
            <c:if test="${param.search!='null'}">
                <%
                try {
                    List<Drivers> l = new DriverDetailsService().search(request.getParameter("search"));
                    request.setAttribute("listDrivers", l);
                } catch(Exception ex) {
                    ex.printStackTrace();		
                }
                %>
        ...
        <c:when test="${param.btn=='edit'}">
            <%
            try {
                request.getRequestDispatcher("DriverDetailsEdit.jsp").forward(request, response);
            } catch(Exception ex) {
                ex.printStackTrace();		
            }
            %>
        ...
        <c:otherwise>
            <%
            List<Drivers> list = new DriverDetailsService().getAll();
            request.setAttribute("listDrivers", list);
            %>
        </c:otherwise>
    </c:choose>
    <div class="container mt-3 table-responsive" style="height:320px">
        <table class="table table-striped table-bordered table-dark">
            <thead>
                <tr>
                    <th class="col s2">Driver</th>
                    <th class="col s2">PhoneNumber</th>
                    <th class="col s2">LicenseNumber</th>
                    <th class="col s2">Bank</th>
                    <th class="col s2">A/CNumber</th>
                    <th class="col s2">PAN</th>
                    <th></th>
                </tr>
            </thead>
            <tbody>
                <c:forEach items="${listDrivers}" var="temp">
                    <tr>
                        <div >
                            <td class="col s2">${temp.name}</td>
                            <td class="col s2">${temp.phoneNumber}</td> 
                             <td class="col s2">${temp.licenseNumber}</td>
                              <td class="col s2">${temp.bankName}</td>
                               <td class="col s2">${temp.bankAccount}</td> 
                                <td class="col s1">${temp.PAN}</td>      
                        </div>
                        <div>
                            <td class="col-1">
                                <form action="DriverDetails.jsp?driverno=${temp.driverNo }" method="post" style="display:inline;">
