...
<nav class="navbar navbar-expand-sm bg-dark navbar-dark">
    <div class="container-fluid">
        ...
        <ul class="navbar-nav ms-auto">
            ...
            <li class="nav-item">
                <a class="nav-link active" href="EmployeesAdd.jsp"><img src="https://icons4web.com/wp-content/uploads/2017/11/Add-internet-icon-190282.jpg" alt="Logo" style="width:40px;" class="rounded-pill"></a>
            </li>
            <li class="nav-item">
...
<div class="content">
    <div class="row">
        <div class="col-sm-12 bg-primary text-white">
            <div class="search-bar">
                <form class="d-flex" action="Employees.jsp?action=search" method="post">
    ...
    <c:choose>
        <c:when test="${param.btn == 'delete'}">
            <c:if test="${param.t1 != 'null'}">
                <% 
                try {
                    new EmployeesService().delete(Integer.parseInt(request.getParameter("empno")));
        ...
        <c:when test="${param.btn=='search'}">
            <c:if test="${param.search!='null'}">
                <%
                try {
                    List<Employees> l = new EmployeesService().search(request.getParameter("search"));
                    request.setAttribute("listEmployees", l);
                } catch(Exception ex) {
                    ex.printStackTrace();		
                }
                %>
        ...
        <c:otherwise>
            <%
            List<Employees> list = new EmployeesService().getAll();
    ...
    <div class="container mt-3 table-responsive" style="height:320px">
        <table class="table table-striped table-bordered table-dark">
            <thead>
                <tr>
                    <th class="col s2">Employee</th>
                    <th class="col s2">Type</th>
                    <th class="col s2">EmailAddress</th>
            ...
            <tbody>
                <c:forEach items="${listEmployees}" var="temp">
                    <tr>
                        <div >
                            <td class="col s2">${temp.name}</td>
                            <td class="col s1">${temp.employeeType}</td> 
                        ...
                        <div>
                            <td class="col-1">
                                <form action="Employees.jsp?empno=${temp.employeeNo }" method="post" style="display:inline;">
