...
<body>
    <div class="content">
        <form action="${pageContext.request.contextPath}/JSP/ShareSourceJSPS/CustomerAddSource.jsp" method="post">
            <div class="container form-container">
                <div class="row">
                    <div class="col-md-6 mb-3">
                        <input type="text" class="form-control custom-input" placeholder="Customer Name" aria-label="Customer Name" name="customername" id="customername">
                    </div>
                    <div class="col-md-6 mb-3">
    ...
    <c:choose>
        <c:when test="${param.btn == 'add'}">
         <jsp:useBean id="customer" class="com.iss.models.Customers" scope="request">
                ...
                <jsp:setProperty name="customer" property="emailAddress" value="${param.emailaddress}" />
                ...
                <jsp:setProperty name="customer" property="pincode" value="${param.Pincode}" />
                <jsp:setProperty name="customer" property="phoneNumber" value="${param.phonenumber}" />
                <jsp:setProperty name="customer" property="mobileNumber" value="${param.mobilenumber}" />
                <jsp:setProperty name="customer" property="username" value="${param.username}" />
                <jsp:setProperty name="customer" property="password" value="${param.pass}" />
            </jsp:useBean>
                <% 
                    try {
                        ...
                        session.setAttribute("loginsuccess","Welcome-"+request.getParameter("customername"));
                       	request.getRequestDispatcher(""+session.getAttribute("path")).forward(request,response);
                    } catch(Exception ex) {
                        ex.printStackTrace();        
                    }
                %>
         ...
         <c:when test="${param.btn=='clear'}">
        	<%
        		try{
        				response.sendRedirect("CustomerAddSource.jsp");
        		}catch(Exception ex) {
                    ex.printStackTrace();        
                }
        	%>
