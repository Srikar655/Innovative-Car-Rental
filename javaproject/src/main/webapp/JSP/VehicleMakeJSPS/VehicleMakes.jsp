...
<nav class="navbar navbar-expand-sm bg-dark navbar-dark">
    <div class="container-fluid">
        <a class="navbar-brand" href="#">VehicleMakeDetails</a>
        <ul class="navbar-nav ms-auto">
            ...
            <li class="nav-item">
                <a class="nav-link active" href="VehicleMakesAdd.jsp"><img src="https://icons4web.com/wp-content/uploads/2017/11/Add-internet-icon-190282.jpg" alt="Logo" style="width:40px;" class="rounded-pill"></a>
            </li>
            <li class="nav-item">
...
<div class="content">
    <div class="row">
        <div class="col-sm-12 bg-primary text-white">
            <div class="search-bar">
                <form class="d-flex" action="VehicleMakes.jsp" method="post">
    ...
    <c:choose>
        <c:when test="${param.btn == 'delete'}">
            <c:if test="${param.t1 != 'null'}">
                <% 
                try {
                    new VehicleMakesService().delete(Integer.parseInt(request.getParameter("conid")));
                    response.sendRedirect("VehicleMakes.jsp");
                } catch(Exception ex) {
                    ex.printStackTrace();		
                }
                %>
        ...
        <c:when test="${param.btn=='search'}">
            <c:if test="${param.search!='null'}">
                <%
                try {
                    List<VehicleMakes> l = new VehicleMakesService().search(request.getParameter("search"));
                    request.setAttribute("listMakes", l);
                } catch(Exception ex) {
                    ex.printStackTrace();		
                }
                %>
        ...
        <c:when test="${param.btn=='edit'}">
            <%
            try {
                request.getRequestDispatcher("VehicleMakesEdit.jsp").forward(request, response);
            } catch(Exception ex) {
                ex.printStackTrace();		
            }
            %>
        ...
        <c:otherwise>
            <%
            	List<VehicleMakes> list = new VehicleMakesService().getAll();
           		 request.setAttribute("listMakes", list);
            %>
        </c:otherwise>
    </c:choose>
    <div class="container mt-3 table-responsive" style="height:320px">
        <table class="table table-striped table-bordered table-dark">
            <thead>
                <tr>
                    <th scope="col-12">Vehicle Make</th>
                    <th></th>
                </tr>
            </thead>
            <tbody>
                <c:forEach items="${listMakes}" var="temp">
                    <tr>
                        <div>
                            <td class="col-11">${temp.name}</td>
                        </div>
                        <div>
                            <td class="col-1">
                                <form action="VehicleMakes.jsp?conid=${temp.makeNo}" method="post" style="display:inline;">
