...
<nav class="navbar navbar-expand-sm bg-dark navbar-dark">
    <div class="container-fluid">
        ...
        <ul class="navbar-nav ms-auto">
            ...
            <li class="nav-item">
                <a class="nav-link active" href="OwnersAdd.jsp"><img src="https://icons4web.com/wp-content/uploads/2017/11/Add-internet-icon-190282.jpg" alt="Logo" style="width:40px;" class="rounded-pill"></a>
            </li>
            <li class="nav-item">
...
<div class="content">
    ...
    <c:choose>
        <c:when test="${param.btn == 'delete'}">
            <c:if test="${param.t1 != 'null'}">
                <% 
                try {
                    new OwnersService().delete(Integer.parseInt(request.getParameter("ownerno")));
        ...
        <c:when test="${param.btn=='search'}">
            <c:if test="${param.search!='null'}">
                <%
                try {
                    List<Owners> l = new OwnersService().search(request.getParameter("search"));
                    request.setAttribute("listOwners", l);
                } catch(Exception ex) {
                    ex.printStackTrace();		
                }
                %>
        ...
        <c:when test="${param.btn=='edit'}">
            <%
            try {
                request.getRequestDispatcher("OwnersEdit.jsp").forward(request, response);
            } catch(Exception ex) {
                ex.printStackTrace();		
            }
            %>
        ...
        <c:otherwise>
            <%
            List<Owners> list = new OwnersService().getAll();
            request.setAttribute("listOwners", list);
            %>
        </c:otherwise>
    </c:choose>
    <div class="container mt-3 table-responsive" style="height:320px">
        <table class="table table-striped table-bordered table-dark">
            ...
            <tbody>
                <c:forEach items="${listOwners}" var="temp">
                    <tr>
                        <div >
                            ...
                            <td class="col s2">${temp.phoneNumber}</td> 
                              <td class="col s2">${temp.bankName}</td>
                               <td class="col s2">${temp.bankAccount}</td> 
                                <td class="col s2">${temp.PAN}</td>      
                        </div>
                        <div>
                            <td class="col-2">
                                <form action="Owners.jsp?ownerno=${temp.ownerNo }" method="post" style="display:inline;">
