...
<nav class="navbar navbar-expand-sm bg-dark navbar-dark">
    <div class="container-fluid">
        <a class="navbar-brand" href="#">CountryDetails</a>
        <ul class="navbar-nav ms-auto">
            ...
            <li class="nav-item">
                <a class="nav-link active" href="CountriesAdd.jsp"><img src="https://icons4web.com/wp-content/uploads/2017/11/Add-internet-icon-190282.jpg" alt="Logo" style="width:40px;" class="rounded-pill"></a>
            </li>
            <li class="nav-item">
    ...
    <div class="row">
        <div class="col-sm-12 bg-primary text-white">
            <div class="search-bar">
                <form class="d-flex" action="Countries.jsp?action=search" method="post">
    ...
    <c:choose>
        <c:when test="${param.btn == 'delete'}">
            <c:if test="${param.t1 != 'null'}">
                <% 
                try {
                    new CountriesService().delete(Integer.parseInt(request.getParameter("conid")));
        ...
        <c:when test="${param.btn=='search'}">
            <c:if test="${param.search!='null'}">
                <%
                try {
                    List<Countries> l = new CountriesService().search(request.getParameter("search"));
                    request.setAttribute("listCountries", l);
                } catch(Exception ex) {
                    ex.printStackTrace();		
                }
                %>
        ...
        <c:when test="${param.btn=='edit'}">
            <%
            try {
                request.getRequestDispatcher("CountriesEdit.jsp").forward(request, response);
            } catch(Exception ex) {
                ex.printStackTrace();		
            }
            %>
        ...
        <c:otherwise>
            ...
            request.setAttribute("listCountries", list);
            %>
        </c:otherwise>
    </c:choose>
	    <div class="container mt-3 table-responsive" style="height:320px">
	        <table class="table table-striped table-bordered table-dark ">
	            <thead>
	                <tr>
	                    <th scope="col-12">Country Name</th>
	                    <th></th>
	                </tr>
	            </thead>
	            <tbody>
	                <c:forEach items="${listCountries}" var="temp">
	                    <tr>
	                        <div>
	                            <td class="col-11">${temp.country}</td>
	                        </div>
	                        <div>
	                            <td class="col-1">
	                                <form action="Countries.jsp?conid=${temp.countryNo}" method="post" style="display:inline;">
